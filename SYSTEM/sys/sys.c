...
uint8_t sys_clock_set(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    PWR->D3CR |= 3 << 14;       /* VOS[1:0] = 3, 电压调节选择Scale1, VCORE = 1.20V */
    ...
    else
    {
        ...
        temp |= 2 << 0;                     /* LATENCY[3:0] = 2, 2个CPU等待周期(@VOS1 Level, maxclock = 210Mhz) */
...
void sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    {
#if defined(GPV)
        GPV->AXI_TARG7_FN_MOD = 0x00000001; /* Only present on some header variants */
#endif
    }    
