...
void lcd_init(void)
{
    ...
    sys_gpio_af_set(LCD_CS_GPIO_PORT, LCD_CS_GPIO_PIN, 12);     /* LCD_CS引脚, AF12 */
    sys_gpio_af_set(LCD_RS_GPIO_PORT, LCD_RS_GPIO_PIN, 12);     /* LCD_RS引脚, AF12 */
    sys_gpio_af_set(LCD_WR_GPIO_PORT, LCD_WR_GPIO_PIN, 12);     /* LCD_WR引脚, AF12 */
    sys_gpio_af_set(LCD_RD_GPIO_PORT, LCD_RD_GPIO_PIN, 12);     /* LCD_RD引脚, AF12 */
    ...
    lcd_opt_delay(0XFFFFF);     /* 初始化FMC后,等待一定时间才能开始初始化 */
...
#if (LCD_FORCE_ID != 0)
    if (lcddev.id == 0xFFFF || lcddev.id == 0x0000)
    {
        lcddev.id = LCD_FORCE_ID;
        printf("LCD force ID:%x\r\n", lcddev.id);
    }
#endif
    ...
    if (lcddev.id == 0X7789)  
    {
        ...
        LCD_FMC_BWTRX &= ~(0XFF << 8);  /* 数据保存时间(DATAST)清零 */
